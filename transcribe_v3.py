...
import os, sys, subprocess, tempfile, time, urllib.request, urllib.error, uuid
...
def extract_audio(mp4_path, audio_path):
    r = subprocess.run(["ffmpeg","-y","-loglevel","error","-i",str(mp4_path),
        "-vn","-acodec","aac","-b:a","64k","-ar","16000","-ac","1",str(audio_path)],
...
def main():
    ...
    for i, mp4 in enumerate(mp4_files, 1):
        ...
        with tempfile.NamedTemporaryFile(suffix=".m4a", delete=False) as tmp:
            ap = Path(tmp.name)
        try:
            if not extract_audio(mp4, ap):
                print("FAIL(audio)")
                failed += 1; continue
            text, err = transcribe(ap)
            if err:
                print(f"FAIL({err})")
                failed += 1; continue
